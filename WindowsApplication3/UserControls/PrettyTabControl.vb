...
Public Class PrettyTabControl
    ...
    Protected Overrides Sub OnPaint(ByVal e As PaintEventArgs)
        ...
        G.Clear(Color.White)
        G.FillRectangle(New SolidBrush(Color.FromArgb(229, 229, 229)), New Rectangle(0, 0, ItemSize.Height + 4, Height)) 'ToolFläche(unten links)
        'G.DrawLine(New Pen(Color.FromArgb(153, 204, 0)), New Point(Width - 1, 0), New Point(Width - 1, Height - 1))    'comment out to get rid of the borders
        'G.DrawLine(New Pen(Color.FromArgb(153, 204, 0)), New Point(ItemSize.Height + 1, 0), New Point(Width - 1, 0))                   'comment out to get rid of the borders
        'G.DrawLine(New Pen(Color.FromArgb(153, 204, 0)), New Point(ItemSize.Height + 3, Height - 1), New Point(Width - 1, Height - 1)) 'comment out to get rid of the borders
        G.DrawLine(New Pen(My.Settings.ColorDark), New Point(ItemSize.Height + 3, 0), New Point(ItemSize.Height + 3, 999))
        For i = 0 To TabCount - 1
            If i = SelectedIndex Then
                ...
                myBlend.Colors = {My.Settings.ColorLight, My.Settings.ColorLight, My.Settings.ColorLight}
                myBlend.Positions = {0.0F, 0.5F, 1.0F}
                Dim lgBrush As New LinearGradientBrush(x2, Color.Black, Color.Black, 90.0F)
                lgBrush.InterpolationColors = myBlend
                G.FillRectangle(lgBrush, x2)
                G.DrawRectangle(New Pen(My.Settings.ColorDark), x2)
                ...
                G.DrawPolygon(New Pen(My.Settings.ColorDark), p)
            ...
            Else
                Dim x2 As Rectangle = New Rectangle(New Point(GetTabRect(i).Location.X - 2, GetTabRect(i).Location.Y - 2), New Size(GetTabRect(i).Width + 3, GetTabRect(i).Height + 1))
                G.FillRectangle(New SolidBrush(Color.FromArgb(221, 221, 221)), x2) 'Deaktivierte Buttons
                G.DrawLine(New Pen(My.Settings.ColorDark), New Point(x2.Right, x2.Top), New Point(x2.Right, x2.Bottom))
                If ImageList IsNot Nothing Then
                    Try
                        If ImageList.Images(TabPages(i).ImageIndex) IsNot Nothing Then
                            G.DrawImage(ImageList.Images(TabPages(i).ImageIndex), New Point(x2.Location.X + 8, x2.Location.Y + 6))
